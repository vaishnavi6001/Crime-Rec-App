        ...
        <div>
            ...
            <div className="sidebar">
                ...
                {crimeForm && <div className="addCrime">
                    ...
                    <input type="text" className="form-control" placeholder="Date(dd/mm/yyyy)..." onChange={(event) => {setDate(event.target.value);console.log(date);}}/>
                    ...
                    {listOfCrimes.map((crime) => {
                        return (
                            <div><br />
                                ...
                                Date: {crime.date}<br />
                                <button onClick={() => {
                                    Axios.get("http://localhost:3001/Map0", {params: {_id: crime._id}})
                                    .then((response) => {

                                        console.log(response.data);
                                        fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${response.data.Location}.json?access_token=${mapboxAPIkey}`)
                                        .then(res => res.json())
                                        .then(data => {
                                            console.log(data);
                                            //setViewport()
